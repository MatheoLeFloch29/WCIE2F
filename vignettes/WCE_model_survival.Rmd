...
This cumulative exposure is then included in a Cox proportional hazards model to estimate the hazard of the event of interest for subject $i$:

\begin{align*}
    \lambda_i(t_{surv}, X, \alpha, \delta) &= \lambda_0(t_{surv}) \exp\left[\alpha_p X_{ip} + WCIE_i \right] \\
    &= \lambda_0(t_{surv}) \exp\left[\alpha_pX_{ip}+\sum_{k=0}^1\delta_k\sum_{t=-10}^0B_k(t)\times normMMSE_i^*(t)\right]
    \label{eq7}
\end{align*}

where $\lambda_0(t_{surv})$ is the baseline hazard function, $\alpha_p$ are the coefficients associated with the covariates $X_{ip}$, and the sum represents the weighted cumulative effect of the past exposure $E_i^*(t)$ corrected for measurement error, over the predefined exposure window $[-S,0]$ relative to time $t$.
...
$$ \lambda_i(t_{surv}, X, \alpha, \delta)=\lambda_0(t_{surv})\exp\left[\alpha_1sex_i+\sum_{k=0}^1\delta_k\sum_{t=-10}^0B_k(t)\times normMMSE_i^*(t)\right]$$
...
In the second step, we reuse the linear mixed-effects model fitted in the exposure stage to estimate the time-varying effect of the exposure on the hazard of physical dependency. Specifically, we apply the $WCEland$ function to combine the exposure model with a Cox proportional hazards model that incorporates a weighted cumulative exposure (WCE) structure. This allows us to capture how past cognitive performance influences the risk of developing dependency over time.
...
m_outcome<-WCEland(mexpo = m_expo,var.time = "time2",time.frame = c(-10,0,1),weightbasis = "NS"
              ...
              ,reg.type = "cox",model = Surv(time2,HIER2)~male+WCIE,n_boot = 10)
```
